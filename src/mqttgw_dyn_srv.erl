...
create_dynsub(Subject, Data, DynsubRespData) ->
    ...
    case vmq_subscriber_db:read({[], Subject}) of
        ...
        [{Node, _, _Subs}] ->
            ...
            start_dynsub(Node, Subject, Topic, Data, DynsubRespData),
            ok
    end.
...
start_dynsub(Node, Subject, Topic, Data, DynsubRespData) ->
    gen_server:cast(
        {global, {?MODULE, Node}},
        {start_dynsub, Subject, Topic, Data, DynsubRespData, node()}
    ).

register_dynsub_in_progress(Node, Subject, Topic, DynsubRespData) ->
    gen_server:cast(
        {global, {?MODULE, Node}},
        {register_dynsub_in_progress, Subject, Topic, DynsubRespData}
    ).
...
handle_cast({start_dynsub, Subject, Topic, Data, DynsubRespData, From}, State) ->
    register_dynsub_in_progress(From, Subject, Topic, DynsubRespData),
    create_sub(Subject, Data),
    {noreply, State};
handle_cast(
    {register_dynsub_in_progress, Subject, Topic, DynsubRespData},
...
addition_fold({Topic, _Qos}, Acc) ->
    case maps:find(Topic, Acc) of
        error -> Acc;
        {ok, {CorrData, RespTopic, SenderConn, SenderId,
            UniqueId, SessionPairId, Time}} ->
            mqttgw:send_dynsub_response(
                CorrData, RespTopic, SenderConn, SenderId,
                UniqueId, SessionPairId, Time),
...
removal_fold({Topic, _Qos}, Acc) ->
    case maps:find(Topic, Acc) of
        error -> Acc;
        {ok, {Subject, Data, {CorrData, RespTopic, SenderConn, SenderId,
            UniqueId, SessionPairId, Time}}} ->
            %% Send a response to the application.
            mqttgw:send_dynsub_response(
                CorrData, RespTopic, SenderConn, SenderId,
            ...
            mqttgw:send_dynsub_multicast_event(
                <<"subscription.delete">>,
                Subject, Data, SenderConn, SenderId,
                UniqueId, SessionPairId, Time),
            maps:remove(Topic, Acc)
    end.
